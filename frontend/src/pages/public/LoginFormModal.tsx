...
const LoginFormModal: React.FC<LoginFormModalProps> = ({ onClose, onLoginSuccess, onSwitchToSignup, onSwitchToForgotPassword }) => {
    ...
    const inputClasses = "w-full bg-[#0D1117] border border-white/10 rounded-xl py-3.5 pl-12 pr-12 text-white placeholder-slate-500 focus:ring-1 focus:ring-cyan-500/50 focus:border-cyan-500/50 outline-none transition-all duration-300 shadow-[inset_0_2px_4px_rgba(0,0,0,0.5)]";
    ...
    return (
        ...
        >
            ...
            <div
                className={`relative w-full max-w-lg bg-[#070F20]/90 backdrop-blur-2xl rounded-3xl border border-white/10 shadow-[0_0_50px_rgba(6,182,212,0.15)] overflow-hidden transform transition-all duration-300 ${isClosing ? 'scale-95 translate-y-4' : 'scale-100 translate-y-0 animate-modal-content-slide-down'}`}
            ...
            >
                ...
                <button onClick={handleClose} className="absolute top-6 right-6 p-2 text-slate-500 hover:text-white transition-colors rounded-full hover:bg-white/10 z-20">
                ...
                <div className="p-8 md:p-12 relative z-10 flex flex-col justify-center">
                    <div className="text-center mb-10">
                        <div className="flex justify-center mb-6">
                            <div className="flex items-center gap-2 mb-2">
                                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-cyan-500 to-violet-600 flex items-center justify-center shadow-[0_0_20px_rgba(6,182,212,0.5)]">
                        ...
                        <p className="text-sm text-slate-400 mt-2">Enter credentials to authenticate into the network.</p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div className="flex justify-between items-center text-sm px-1">
                            <label className="flex items-center space-x-2.5 cursor-pointer">
                                <input type="checkbox" className="w-4 h-4 rounded border-white/20 bg-transparent text-cyan-500 focus:ring-cyan-500/50 focus:ring-offset-0 transition-colors" />
                                <span className="text-slate-400">Remember credentials</span>
                            </label>
                            <button type="button" onClick={onSwitchToForgotPassword} className="text-cyan-400 hover:text-cyan-300 transition-colors">Access Recovery?</button>
                        </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <button className="flex items-center justify-center py-3 border border-white/10 bg-white/5 rounded-xl hover:bg-white/10 hover:border-cyan-500/30 transition-all group">
                            <GoogleLogo className="grayscale group-hover:grayscale-0 transition-all" />
                        </button>
                        <button className="flex items-center justify-center py-3 border border-white/10 bg-white/5 rounded-xl hover:bg-white/10 hover:border-cyan-500/30 transition-all group">
                            <GithubLogo className="text-slate-400 group-hover:text-white transition-all" />
                        </button>
                    </div>

                    <div className="mt-10 text-center text-sm text-slate-500">
                        No active credentials?{' '}
                        <button onClick={onSwitchToSignup} className="font-bold text-cyan-400 hover:text-cyan-300 hover:underline decoration-cyan-400/30 underline-offset-4 transition-all">
